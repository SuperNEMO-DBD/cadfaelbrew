...
module Homebrew
  def update
    ...
    # Ensure our required taps are tapped - always full for main one
    ARGV.push("--full")
  ...
  def git_init_if_necessary
    if Dir[".git/*"].empty?
      ...
      safe_system "git", "config", "remote.origin.url", "https://github.com/#{OS::GITHUB_USER}/#{OS::GITHUB_REPOSITORY}.git"
    ...
    if `git remote show origin -n` =~ /Fetch URL: \S+mxcl\/homebrew/
      safe_system "git", "remote", "set-url", "origin", "https://github.com/#{OS::GITHUB_USER}/#{OS::GITHUB_REPOSITORY}.git"
